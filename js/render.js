// render.js — SVG rendering: components, wires, terminals, badges, coordinate transforms
  ...
  function renderPsu24(c){
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="45" y="30" text-anchor="middle" font-size="7" fill="#9CA3AF">Control Supply</text>
      ${terms}
    </g>`;
  }
  ...
  function renderContactor(c){
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="115" y="63" class="chip-label" font-size="10.5">Contactor${en?'  ● ON':''}</text>
  ...
  function renderMotor(c){
    ...
    const dirLabel = dir==='rev' ? '⟲ REVERSE' : dir==='fwd' ? '⟳ FORWARD' : '';
    const dirColor = dir==='rev' ? '#F5A623' : '#22C55E';
    const jumper = motorJumperStatus(c);
    const jumperLabel = jumper==='delta' ? 'Δ jumper ✓' : jumper==='star' ? 'Y jumper ✓'
      : jumper==='delta-partial' ? 'Δ partial ⚠' : jumper==='star-partial' ? 'Y partial ⚠'
      : 'Y/Δ no jumper ⚠';
  ...
  function renderBreaker(c){
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="70" y="42" text-anchor="middle" font-size="8" fill="#4B5563" font-weight="700">Circuit Breaker</text>
      ${toggles}
  ...
  function renderRelay(c){
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="70" y="57" class="chip-label" font-size="8.5">Thermal Relay${tripped?' ⚠':''}</text>
  ...
  function renderEstop(c){
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="45" y="140" class="chip-label" font-size="8">${tripped?'LOCKED':'E-STOP'}</text>
  ...
  function renderTimer(c){
    ...
    const status = done ? 'DONE ✓' : on ? 'TIMING...' : 'OFF';
    ...
    return `<g class="comp" data-comp="${c.id}" transform="translate(${c.x},${c.y})">
      ...
      <text x="75" y="57" class="chip-label" font-size="9">Timer Relay (${c.delaySec||3}s)</text>
